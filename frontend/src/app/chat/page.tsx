  ...
  const handleHitlDecision = (decision: "allow" | "deny") => {
    ...
    setMessages((prev) => [
      ...prev,
      {
        ...
        type: "user",
        content: decision === "allow" ? "You allowed the risky action." : " You denied the risky action."
      }
    ]);
  };
    ...
    <div className="flex flex-col h-screen bg-white text-gray-800 font-sans">
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-white/80 backdrop-blur-sm sticky top-0 z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/voice"
            className="flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 shadow-sm transition-colors hover:border-emerald-300 hover:text-emerald-600"
            title="talk to piloteer"
          >
