...
loggings/
    database/
        piloteer_logs.db        SQLite database — one row per agent event
    screenshots/
        run_<id>/
            subgoal_000_step_000_before.png
            ...
    scripts/
        schema.py               Database path constants and init_db()
        logger.py               async log_event() function
        screenshot.py           Path helpers for screenshot files
```
...
Screenshots are only taken by the Actor node, once per step, immediately before the browser action is executed. This captures exactly what the agent saw when it made its decision. The screenshot from step N is effectively the "after" state of step N-1, so no duplication is needed. Screenshots are stored as files on disk rather than as BLOBs in the database, and only the file path is stored in the events table. This keeps the database small and fast.
...
- **Sufficient throughput** — Piloteer produces at most a few dozen rows per minute. SQLite handles thousands of writes per second, so it is never a bottleneck.
...
A server-based database (PostgreSQL, MySQL) would only make sense if multiple Piloteer instances were writing concurrently or if the dataset exceeded hundreds of millions of rows. Neither applies here.
